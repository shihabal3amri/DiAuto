...
class Settings(private val context: Context) {
    ...
    var useMeasuredTouchSurface: Boolean
        get() = prefs.getBoolean("use_measured_touch_surface", true)
    ...
    var insetBottom: Int
        get() = prefs.getInt("inset-bottom", 0) // DiLink fullscreen is handled by BydFullscreen (no letterbox inset)
    ...
    var debugVideoFaultInjection: VideoFaultInjector.Mode
        get() {
            if (!com.andrerinas.openheadunit.BuildConfig.DEBUG) return VideoFaultInjector.Mode.OFF
    ...
    var vehicleDisplayName: String
        ...
        get() = prefs.getString("vehicle-display-name", "DiAuto")!!
    ...
    var wifiConnectionMode: Int
        get() {
            ...
            return prefs.getInt("wifi-connection-mode", 3) // Native Android Auto; no companion app
        }
    ...
    var primaryConnection: ConnectionKind
        get() = ConnectionKind.fromInt(prefs.getInt("primary-connection", ConnectionKind.NATIVE_AA.value))
    ...
    var autoConnectLastSession: Boolean
        get() = prefs.getBoolean("auto-connect-last-session", true)
    ...
    val displayRefreshRate: Float
        get() = (context.getSystemService(Context.WINDOW_SERVICE) as android.view.WindowManager)
    ...
    var autoStartOnBoot: Boolean
        get() = prefs.getBoolean("auto-start-on-boot", true)
